...
class Controller_Expertsf extends Controller_Template_Main {
	...
	public function action_index()
	{
		...
		$this->template->content = View::factory('expertsf/experts_list')
			->bind('data', $data)
			->bind('alfabet', $alfabet)
			->bind('letter', $letter)	
			->bind('search', $search)
			->set('url_expert_only', '/expertsf')
		;
        $this->template->url = 'expertsf';
    }
	...
	public function action_experts_only()
	{
		...
		//Модель
		$db_data = Model::factory('Expertsf')->get_expert_only($expert_url);
				
		
		//Хлебные крошки
		$this->template->breadcrumb = array('expertsf' => 'Наши эксперты', $db_data['familia'].' '.$db_data['name'].' '.$db_data['otchestvo']);
		//Заголовки и т.п.
		...
		$this->template->content = View::factory('expertsf/experts_only')->bind('db_data', $db_data);
	}
