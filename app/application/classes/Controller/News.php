...
class Controller_News extends Controller_Template_Main {
	...
	{
		$this->template->breadcrumb = array( 'Новости' );
		...
		$newstags = ORM::factory('NewsTagsName')
			->where('razdel_id', 'IN', array(0, 1, 2))
			->order_by('sort', 'ASC')
			->where('active', '=', '1')
			->find_all();
	...
	public function action_news_only()
	{
		...
		//Хлебные крошки
		$this->template->breadcrumb = array('news' => 'Новости', $news_data['title']);
		//Заголовки и т.п.
