...
class Controller_PageStatic extends Controller_Template_Main {
	...
	private function get_static_content($url = null)
	{
		...
		if( $page->pagestatic->id > 0 && $page->pagestatic->active == 1 )
		{
            ...
            if( preg_match('#<!--module (.*)-->#iu', $this->template->content, $module)  ) {
                ...
                $response = Request::factory( 'module/'.$module[1] )->execute();

              

                //$mod = view::factory('module/'.$module[1]);
                $this->template->content = str_replace($module[0], $response, $this->template->content);
            }
