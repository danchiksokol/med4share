...
class Controller_Admin_Expertsf extends Controller_Admin_Admin {
	...
	public function action_experts()
	{
		...
		$content = View::factory('admin/expertsf/experts_list')
            //->bind('blocks', $blocks)
            ->bind('expertsf', $data)
            ;
	...
	public function action_add() {
		...
		{
            $model_experts = new Model_Expertsf();
            ...
            Controller::redirect('admin/expertsf/edit/'.$last_insert_id);
        }
		
		$this->template->breadcrumb = array('admin/expertsf/add' => 'Добавление нового Эксперта');
        $this->template->content =  View::factory('admin/expertsf/experts_add');
	...
	public function action_edit() {
        ...
        if($expert_id) {
            //Достаем контент
            $data = ORM::factory ('Expertsf', $expert_id);
        }
		...
		$this->template->breadcrumb = array('admin/expertsf/edit/'.$expert_id => 'Редактирование Эксперта');
        $this->template->content =  View::factory('admin/expertsf/experts_edit')
         							->bind('data', $data);
	}
	...
	public function action_delete() {
        if($this->request->is_ajax() && $this->request->post()){
            $this->auto_render = false;
            ORM::factory('Expertsf', $this->request->post('experts_id') )->delete();
    ...
    public function action_active()
    {
        if($this->request->is_ajax() && $this->request->post()){
            ...
            $active = DB::query(Database::UPDATE, "UPDATE `expertsf` SET active = NOT active, modified_date = '".date('Y-m-d H:i:s')."' WHERE id =". $this->request->post('experts_id'))->execute();
