...
class Controller_Neurology_Experts extends Controller_Template_Main
{
    ...
    {
        $this->template->site_razdel_id = 4;
    ...
    public function action_index()
    {
        ...
        $data = ORM::factory('Experts')
            ...
            ->and_where('active', '=', 1)
            ->and_where('razdel_id', 'IN', array(0, 4) )
            ->order_by('familia', 'ASC')
            ->find_all();


        $alfabet = ORM::factory('Experts')->getALfabet( 4 );


        //Передаем всё в шаблон
        $this->template->breadcrumb = array( 'neurology' => 'Неврология', 'Наши эксперты' );
        ...
        $this->template->content = View::factory('experts/experts_list')
            ...
            ->bind('letter', $letter)
            ->bind('search', $search)
            ->set('url_expert_only', '/neurology/experts')
        ;
    ...
    public function action_experts_only() {
        ...
        //Хлебные крошки
        $this->template->breadcrumb = array('neurology' => 'Неврология', 'neurology/experts' => 'Наши эксперты', $db_data['familia'].' '.$db_data['name'].' '.$db_data['otchestvo']);
        //Заголовки и т.п.
