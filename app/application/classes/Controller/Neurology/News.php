...
class Controller_Neurology_News extends Controller_Template_Main {
	...
	public function after()
	{
		...
		$this->template->site_razdel = 'neurology';
		parent::after();
	}
	...
	{
		$this->template->breadcrumb = array( 'neurology' => 'Неврология', 'Новости' );
		...
		$this->template->content .= View::factory('news/news_list')
			...
			->set('url_news_only', '/neurology/news')
			;

		$this->template->url = 'neurology/news';
	}
	...
	public function action_news_only()
	{
		...
		//Хлебные крошки
		$this->template->breadcrumb = array('neurology' => 'Неврология', 'neurology/news' => 'Новости', $news_data['title']);
		//Заголовки и т.п.
