 ...
 */
class Model_Experts extends ORM {
	
	protected $_table_name = 'experts'; // <== вручную установить имя таблицы
	...
	{
		return DB::query(Database::SELECT, "SELECT * FROM `experts` WHERE url = :url AND `active` = 1 LIMIT 1")
					->param(':url', $url)
					->execute()
					->current();	
	}
	...
	{
		$update = ORM::factory('Experts', $data['id']);
		//Удаляем не нужное
		unset($data['id']);
		// дата обновления
